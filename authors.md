# Authors and Citation

## Authors

- **Sounkou Mahamane Toure**. Author, maintainer.

- **Simone Rubinacci**. Contributor, copyright holder.  
  GLIMPSE2 upstream implementation

- **Olivier Delaneau**. Contributor, copyright holder.  
  GLIMPSE2 upstream implementation

- **Evan Nemerson**. Copyright holder.  
  SIMDe copyright holder

- **Boost Software Authors**. Contributor, copyright holder.  
  Boost C++ Libraries authors and copyright holders

## Citation

Source:
[`DESCRIPTION`](https://github.com/RGenomicsETL/RGlimpse2/blob/main/DESCRIPTION)

Toure SM (2026). *RGlimpse2: An 'R' Wrapper for 'GLIMPSE2' Low-Coverage
Sequencing Imputation and Phasing*. R package version 0.0.0.9003,
<https://github.com/RGenomicsETL/RGlimpse2>.

    @Manual{,
      title = {RGlimpse2: An 'R' Wrapper for 'GLIMPSE2' Low-Coverage Sequencing Imputation and Phasing},
      author = {Sounkou Mahamane Toure},
      year = {2026},
      note = {R package version 0.0.0.9003},
      url = {https://github.com/RGenomicsETL/RGlimpse2},
    }
